...
class SymmetricConv2d(nn.Conv2d):
    def __init__(
        ...
        if symmetry is None:
            ...
            assert sum(list(
                symmetry.values())) <= self.out_channels, "Number of symmetric channels exceeds number of out channels"
            self.unique_out_channels = self.out_channels - symmetry['h'] // 2 - symmetry['v'] // 2 - 3 * symmetry[
                'hv'] // 4
    ...
    def forward(self, input_):
        ...
        return self.conv2d_forward(input_, torch.cat(weight, dim=0))
...
class SymmetricConv3d(nn.Conv3d):
    ...
    def forward(self, input_):
        ...
        if s['hvz'] > 0:
            ...
            ix += n
        return self.conv3d_forward(input_, torch.cat(weight, dim=0))
